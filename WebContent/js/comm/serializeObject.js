/*
 * .serializeObject (c) Dan Heberden danheberden.com
 * 
...
(function($) {
	$.fn.serializeObject = function() {
		...
		$el.find(':input[type!="checkbox"][type!="radio"], input:checked')
				.each(function() {
					// data[a][b] becomes [ data, a, b ]
					var named = this.name.replace(/\[([^\]]+)?\]/g, ',$1')
							.split(','), cap = named.length - 1, i = 0;

					// Ensure that only elements with valid `name` properties
					// will be serialized
					if (named[0]) {
						for (; i < cap; i++) {
